-- Track first WO PDF download for chip state.
ALTER TABLE jobs ADD COLUMN IF NOT EXISTS last_downloaded_at TIMESTAMPTZ;

COMMENT ON COLUMN jobs.last_downloaded_at IS
  'Set when the work-order PDF is first downloaded. Surfaces a "Downloaded" chip until esign/offline sign progresses the state.';

DROP FUNCTION IF EXISTS public.list_work_orders_dashboard(uuid, uuid[]);

CREATE FUNCTION public.list_work_orders_dashboard(
  p_user_id uuid,
...
CREATE FUNCTION public.list_work_orders_dashboard_page(
  p_user_id uuid,
  p_limit integer,
