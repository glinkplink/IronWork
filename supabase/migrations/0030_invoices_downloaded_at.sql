-- Split download tracking from issuance so a downloaded-but-not-sent invoice can show "Downloaded".
ALTER TABLE invoices ADD COLUMN IF NOT EXISTS downloaded_at TIMESTAMPTZ;

COMMENT ON COLUMN invoices.downloaded_at IS
  'Set on first successful PDF download. Independent from issued_at (which is set on send). Drives "Downloaded" chip when issued_at is null.';
