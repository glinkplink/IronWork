...
- **Invoice lifecycle:** `issued_at = null` → Draft; **`issued_at` is set only on the first successful `POST /api/invoices/:id/send`** (email-only or `include_payment_link: true`). **PDF download sets `downloaded_at`** via `POST /api/invoices/:id/mark-downloaded` (idempotent; same signature + pending-CO gates as send) so downloaded-but-unsent invoices can show a **Downloaded** chip without becoming issued. **`POST /api/stripe/invoices/:id/payment-link` alone does not set `issued_at`.** `payment_status` / `paid_at` are updated by the Stripe webhook.
