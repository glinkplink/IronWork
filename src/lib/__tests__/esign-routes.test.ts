...
const INVOICE_UUID = '880e8400-e29b-41d4-a716-446655440003';
...
describe('tryHandleEsignRoute', () => {
  ...
  it('invoice send POSTs DocuSeal payload with submitter external_id = invoice id and stamps issued_at', async () => {
    const res = captureRes();
    const updatedRow = {
      ...baseInvoiceRow(),
      ...
      esign_submitter_id: '77',
      esign_status: 'sent',
      esign_sent_at: '2026-03-28T15:00:00Z',
    };
        ...
        if (table === 'invoices') {
          return {
            select: vi.fn(() => ({
              eq: vi.fn(() => ({
                eq: vi.fn(() => ({
                  maybeSingle: vi.fn(async () => ({ data: baseInvoiceRow(), error: null })),
                })),
              })),
            })),
            update: vi.fn(() => ({
    ...
    vi.mocked(globalThis.fetch).mockImplementation(async (input: string | URL | Request, init?: RequestInit) => {
      ...
      if (u.includes('/submissions/html') && init?.method === 'POST') {
        const body = JSON.parse(String(init.body)) as { submitters?: { external_id?: string }[] };
        expect(body.submitters?.[0]?.external_id).toBe(INVOICE_UUID);
        const submission = docusealSubmissionResponse(900);
        submission.submitters[0].external_id = INVOICE_UUID;
        return new Response(JSON.stringify(submission), {
          status: 200,
          headers: { 'Content-Type': 'application/json' },
        });
    ...
    const req = {
      method: 'POST',
      url: `/api/esign/invoices/${INVOICE_UUID}/send`,
      headers: { authorization: 'Bearer good-token' },
    };
    await tryHandleEsignRoute(
      req as never,
      res as never,
      defaultHelpers(async () => ({
        documents: [{ html: '<p>Invoice</p>' }],
      }))
    );

    expect(res.status).toBe(200);
    const body = JSON.parse(res.body) as { invoiceId?: string; issued_at?: string; esign_submission_id?: string };
    expect(body.invoiceId).toBe(INVOICE_UUID);
    expect(body.issued_at).toBe('2026-03-28T15:00:00Z');
    expect(body.esign_submission_id).toBe('900');
  });

  it('invoice resend preserves existing issued_at', async () => {
    const res = captureRes();
    const existingIssuedAt = '2026-03-20T08:00:00Z';
    const invoiceWithSubmitter = baseInvoiceRow({
      issued_at: existingIssuedAt,
      esign_submission_id: '900',
      esign_submitter_id: '77',
      esign_status: 'sent',
      esign_sent_at: '2026-03-20T08:00:00Z',
    });
    const updatedRow = {
      ...invoiceWithSubmitter,
      esign_status: 'sent',
      esign_sent_at: '2026-03-28T15:00:00Z',
      esign_submission_state: 'pending',
      esign_submitter_state: 'sent',
    };
    ...
    createClientMock.mockReturnValue({
      ...
      from: vi.fn(() => ({
        select: vi.fn(() => ({
          eq: vi.fn(() => ({
            eq: vi.fn(() => ({
              maybeSingle: vi.fn(async () => ({ data: invoiceWithSubmitter, error: null })),
            })),
          })),
        })),
        update: vi.fn(() => ({
    ...
    vi.mocked(globalThis.fetch).mockImplementation(async (input: string | URL | Request, init?: RequestInit) => {
      const u = requestUrl(input);
      if (u.includes('/submitters/77') && init?.method === 'PUT') {
    ...
    const req = {
      method: 'POST',
      url: `/api/esign/invoices/${INVOICE_UUID}/resend`,
      headers: { authorization: 'Bearer good-token' },
    };
    await tryHandleEsignRoute(req as never, res as never, defaultHelpers());
    expect(res.status).toBe(200);
    const body = JSON.parse(res.body) as { invoiceId?: string; issued_at?: string; esign_sent_at?: string };
    expect(body.invoiceId).toBe(INVOICE_UUID);
    expect(body.issued_at).toBe(existingIssuedAt);
    expect(body.esign_sent_at).toBe('2026-03-28T15:00:00Z');
  });

  it('invoice status fetches DocuSeal, updates invoices, and backfills issued_at when missing', async () => {
    const res = captureRes();
    const invoiceWithSubmission = baseInvoiceRow({
      issued_at: null,
      esign_submission_id: '900',
      esign_submitter_id: '77',
      esign_status: 'sent',
    ...
    const updatedRow = {
      ...invoiceWithSubmission,
      issued_at: '2026-03-28T15:00:00Z',
      esign_status: 'opened',
      esign_submitter_state: 'opened',
      esign_sent_at: '2026-03-28T15:00:00Z',
      esign_opened_at: '2026-03-28T15:05:00Z',
    };
      ...
      from: vi
        ...
        .mockImplementationOnce(() => ({
          select: vi.fn(() => ({
            eq: vi.fn(() => ({
              eq: vi.fn(() => ({
                maybeSingle: vi.fn(async () => ({ data: invoiceWithSubmission, error: null })),
              })),
            })),
          })),
        }))
    ...
    submission.submitters[0].external_id = INVOICE_UUID;
    submission.submitters[0].status = 'opened';
    submission.submitters[0].opened_at = '2026-03-28T15:05:00Z';
    submission.submitters[0].sent_at = '2026-03-28T15:00:00Z';
    vi.mocked(globalThis.fetch).mockResolvedValue(
      new Response(JSON.stringify(submission), {
        status: 200,
        headers: { 'Content-Type': 'application/json' },
    ...
    const req = {
      method: 'GET',
      url: `/api/esign/invoices/${INVOICE_UUID}/status`,
      headers: { authorization: 'Bearer good-token' },
    };
    await tryHandleEsignRoute(req as never, res as never, defaultHelpers());
    expect(res.status).toBe(200);
    const body = JSON.parse(res.body) as { invoiceId?: string; esign_status?: string; issued_at?: string; esign_opened_at?: string };
    expect(body.invoiceId).toBe(INVOICE_UUID);
    expect(body.esign_status).toBe('opened');
    expect(body.issued_at).toBe('2026-03-28T15:00:00Z');
    expect(body.esign_opened_at).toBe('2026-03-28T15:05:00Z');
  });
});
