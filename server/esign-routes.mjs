...
async function findInvoiceForWebhook(supabase, column, value) {
  if (value == null) return null;
  const matchValue = String(value).trim();
  if (!matchValue) return null;
  const { data: row } = await supabase
...
async function resolveInvoiceForFormWebhook(supabase, webhookData) {
  const d = webhookData || {};
  const payloadExternalId = d.external_id;
  if (payloadExternalId && isUuid(payloadExternalId)) {
    const row = await findInvoiceForWebhook(supabase, 'id', payloadExternalId);
    if (row) {
      return { invoice: row, resolvedBy: 'external_id' };
    }
  }

  const submitterId = d.id ?? null;
  if (submitterId != null) {
    const row = await findInvoiceForWebhook(supabase, 'esign_submitter_id', submitterId);
    if (row) {
      return { invoice: row, resolvedBy: 'submitter_id' };
    }
  }

  const payloadSubmissionId = d.submission?.id ?? null;
  if (payloadSubmissionId != null) {
    const row = await findInvoiceForWebhook(supabase, 'esign_submission_id', payloadSubmissionId);
    if (row) {
      return { invoice: row, resolvedBy: 'submission_id' };
    }
  }
...
async function resolveInvoiceForVerifiedSubmissionWebhook(supabase, webhookData, verifiedSubmission) {
  const d = webhookData || {};
  const verifiedSubmissionId = verifiedSubmission?.id;
  if (verifiedSubmissionId != null) {
    const row = await findInvoiceForWebhook(supabase, 'esign_submission_id', verifiedSubmissionId);
    if (row) {
      return { invoice: row, resolvedBy: 'submission_id' };
    }
  }
  ...
  const verifiedExternalId = pickCustomerSubmitter(verifiedSubmission)?.external_id;
  if (verifiedExternalId && isUuid(verifiedExternalId)) {
    const row = await findInvoiceForWebhook(supabase, 'id', verifiedExternalId);
    if (row) {
      return { invoice: row, resolvedBy: 'verified_external_id' };
    }
  }
  ...
  if (payloadExternalId && isUuid(payloadExternalId)) {
    const row = await findInvoiceForWebhook(supabase, 'id', payloadExternalId);
    if (row) {
      return { invoice: row, resolvedBy: 'payload_external_id' };
    }
  }
...
function matchEsignPath(method, pathname) {
  ...
  const invoiceStatusMatch = pathname.match(/^\/api\/esign\/invoices\/([0-9a-fA-F-]{36})\/status$/);
  if (method === 'GET' && invoiceStatusMatch) {
    return { kind: 'invoice-esign-status', invoiceId: invoiceStatusMatch[1] };
  }
  const invoiceMatch = pathname.match(/^\/api\/esign\/invoices\/([0-9a-fA-F-]{36})\/(send|resend)$/);
  if (method === 'POST' && invoiceMatch) {
    return { kind: 'invoice-esign', invoiceId: invoiceMatch[1], action: invoiceMatch[2] };
  }
  return null;
}
...
function publicInvoiceEsignPayload(row) {
  return {
...
function invoiceIssuedAtPatch(invoice, patch) {
  if (!patch) return null;
  if (invoice?.issued_at) return { ...patch, issued_at: invoice.issued_at };
  return {
    ...patch,
    issued_at: patch.esign_sent_at || new Date().toISOString(),
  };
}
...
async function handleInvoiceSend(req, res, readJsonBody, sendJson, sendText, invoiceId) {
  const token = getBearerToken(req);
  if (!token) {
    sendJson(res, 401, { error: 'Missing or invalid Authorization bearer token.' });
    return;
  ...
  const payload = {
    name: body.name || `Invoice #${String(invoice.invoice_number ?? '').padStart(4, '0')}`,
    send_email: body.send_email !== false,
    documents,
    submitters: [
      {
  ...
  const { data: updated, error: upErr } = await supabase
    .from('invoices')
    .update(invoiceIssuedAtPatch(invoice, patch))
    .eq('id', invoiceId)
    .eq('user_id', userId)
    .select('*')
    .single();
  ...
  if (upErr || !updated) {
    sendJson(res, 500, { error: upErr?.message || 'Failed to update invoice after DocuSeal send.' });
    return;
  }
  ...
  sendJson(res, 200, {
    ...
    ...publicInvoiceEsignPayload(updated),
  });
}

async function handleInvoiceResend(req, res, readJsonBody, sendJson, invoiceId) {
  const token = getBearerToken(req);
  if (!token) {
    sendJson(res, 401, { error: 'Missing or invalid Authorization bearer token.' });
    return;
  ...
  const submitterId = invoice.esign_submitter_id;
  if (!submitterId) {
    sendJson(res, 400, { error: 'No signature request to resend. Send first.' });
    return;
  }
  ...
  let reconcileFromSubmission = false;
  let resendPutSucceeded = false;
  try {
    await docusealFetchJson(`/submitters/${submitterId}`, {
      method: 'PUT',
      body: JSON.stringify(putBody),
    });
    resendPutSucceeded = true;
  } catch (e) {
    if ((e?.status === 404 || e?.status === 422) && invoice.esign_submission_id) {
      reconcileFromSubmission = true;
    } else {
      const status = e.status && e.status >= 400 && e.status < 600 ? e.status : 502;
      sendJson(res, status, { error: e instanceof Error ? e.message : 'DocuSeal resend failed.' });
  ...
  let submission = null;
  if (invoice.esign_submission_id) {
    try {
      submission = await docusealFetchJson(`/submissions/${invoice.esign_submission_id}`, {
        method: 'GET',
      });
    } catch (submissionErr) {
      if (reconcileFromSubmission) {
  ...
  if (submission) {
    const patch = invoiceIssuedAtPatch(invoice, buildEsignRowFromSubmission(submission));
    if (patch) {
      if (resendPutSucceeded) {
        patch.esign_sent_at = new Date().toISOString();
      }
      const { data: updated, error: upErr } = await supabase
        .from('invoices')
  ...
  if (resendFallbackPatch) {
    const { data: fallbackUpdated, error: fbErr } = await supabase
      .from('invoices')
      .update(invoiceIssuedAtPatch(invoice, resendFallbackPatch))
      .eq('id', invoiceId)
      .eq('user_id', userId)
      .select('*')
      .single();
    if (!fbErr && fallbackUpdated) {
      sendJson(res, 200, {
        invoiceId,
        jobId: invoice.job_id,
        ...publicInvoiceEsignPayload(fallbackUpdated),
      });
      return;
    }
  }
...
async function handleInvoiceStatus(req, res, sendJson, invoiceId) {
  const token = getBearerToken(req);
  if (!token) {
    sendJson(res, 401, { error: 'Missing or invalid Authorization bearer token.' });
    return;
  ...
  if (!invoice.esign_submission_id) {
    sendJson(res, 200, {
      invoiceId,
      jobId: invoice.job_id,
      ...publicInvoiceEsignPayload(invoice),
    });
    return;
  }
  ...
  preserveDbEsignSentAtIfNewer(patch, invoice);
...
async function handleWebhook(req, res, readJsonBody, sendJson, sendText) {
  ...
  if (formEvent) {
    ...
    if (!resolved) {
      ...
      const invoiceResolved = await resolveInvoiceForFormWebhook(supabase, data);
      if (invoiceResolved) {
        try {
          verifiedInfo = await verifySubmissionForFormWebhook(
            { esign_submission_id: invoiceResolved.invoice.esign_submission_id },
            data
          );
        } catch (e) {
          console.error('DocuSeal verify submission failed for invoice:', e);
          sendJson(res, 502, { error: 'Could not verify submission with DocuSeal.' });
          return;
        }
        if (!verifiedInfo || !verifiedInfo.verified) {
          console.log('[webhook] ignored: could not determine submission to verify (invoice)', {
            invoiceId: String(invoiceResolved.invoice.id),
            submitterId: submitterId != null ? String(submitterId) : null,
          });
          sendJson(res, 200, { ok: true, ignored: true });
          return;
        }
        await handleInvoiceWebhookUpdate(
          supabase,
          invoiceResolved.invoice,
          verifiedInfo,
          invoiceResolved.resolvedBy,
          sendJson,
          res
        );
        return;
    ...
    if (!resolved) {
      ...
      const invoiceResolved = await resolveInvoiceForVerifiedSubmissionWebhook(
        supabase,
        data,
...
async function handleInvoiceWebhookUpdate(supabase, invoice, verifiedInfo, resolvedBy, sendJson, res) {
  if (!verifiedInfo || !verifiedInfo.verified) {
    console.log('[webhook] invoice ignored: could not verify submission');
    sendJson(res, 200, { ok: true, ignored: true });
    return;
  }
  ...
  if (
    ...
    console.log('[webhook] invoice ignored: stale submission', {
      submissionId: String(verified.id),
      payloadSubmissionId: String(verifiedInfo.payloadSubmissionId),
  ...
  if (
    resolvedBy !== 'submission_id' &&
    invoice.esign_submission_id &&
    String(verified.id) !== String(invoice.esign_submission_id)
  ) {
    console.log('[webhook] invoice ignored: stale submission', {
      submissionId: String(verified.id),
      invoiceId: String(invoice.id),
      invoiceSubmissionId: String(invoice.esign_submission_id),
      resolvedBy,
    });
    sendJson(res, 200, { ok: true, ignored: true, reason: 'stale_submission' });
    return;
  ...
  const patch = invoiceIssuedAtPatch(invoice, buildEsignRowFromSubmission(verified));
  if (!patch) {
    console.log('[webhook] invoice ignored: no patch from submission');
    sendJson(res, 200, { ok: true, ignored: true });
    return;
  }

  console.log('[webhook] invoice derived patch', {
  ...
  if (upErr) {
    ...
    console.log('[webhook] invoice update failed', {
      invoiceId: invoice.id,
      esign_status: patch.esign_status,
      error: upErr.message,
    });
  ...
  console.log('[webhook] invoice update applied', {
    invoiceId: invoice.id,
    esign_status: patch.esign_status,
    issued_at: patch.issued_at,
  });
...
export async function tryHandleEsignRoute(req, res, helpers) {
  ...
  try {
    ...
    if (route.kind === 'invoice-esign' && route.action === 'send') {
      await handleInvoiceSend(req, res, readJsonBody, sendJson, sendText, route.invoiceId);
      return true;
    }
    if (route.kind === 'invoice-esign' && route.action === 'resend') {
      await handleInvoiceResend(req, res, readJsonBody, sendJson, route.invoiceId);
      return true;
    }
    if (route.kind === 'invoice-esign-status') {
      await handleInvoiceStatus(req, res, sendJson, route.invoiceId);
      return true;
    }
  } catch (e) {
